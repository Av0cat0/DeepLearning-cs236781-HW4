...
class PolicyAgent(object):
    ...
    def step(self) -> Experience:
        ...
        opt_action = torch.multinomial(actions_proba, 1).item()
        next_state, reward, is_done, _ = self.env.step(opt_action)
        experience = Experience(next_state, opt_action, reward, is_done)
        self.curr_state = torch.tensor(next_state, device=self.device, dtype=torch.float)
        # ========================
        if is_done:
            self.reset()
        return experience
...
class VanillaPolicyGradientLoss(nn.Module):
    ...
    def _policy_loss(self, batch, action_scores, policy_weight):
        ...
        proba_taken = log_proba.gather(index=actions.reshape(-1, 1), dim=1).squeeze(dim=1)
        loss_p = -1.0 * ((proba_taken * policy_weight)).sum(dim=-1) / N
        # ========================
        return loss_p
